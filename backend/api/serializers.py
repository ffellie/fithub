...
class RoomsSerializer2(serializers.ModelSerializer):
    id = serializers.IntegerField(read_only=True)
    number = serializers.IntegerField(required=True)
    isadministrative = serializers.BooleanField(required=True)
    postfix = serializers.CharField(required=False)
    isoccupied = serializers.BooleanField(required=False)

    class Meta:
        model = Rooms
        fields = ('id', 'number', 'isadministrative', 'postfix', 'isoccupied')
